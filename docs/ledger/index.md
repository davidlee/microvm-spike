...
| [36](./036-a-policy-is-selected-not-named.md) | a policy is selected from a declared set, never named by a project | designed, **two of four slices built** and unswitched — the vocabulary and each slot's set, and the allowlist; the collect limbs and the `policy` verb are open |
