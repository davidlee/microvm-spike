...
| [41](./041-a-delegable-verb-that-ends-in-root.md) | a delegable verb that ends in root, and the half-write when it cannot | **open** — `capsule <slot> policy <name>` ends in `sudo systemctl restart capsule-proxy-<slot>` and this host permits no such rule; the branch had never run, and ran today only on a sudo ticket a `just up` left warm. The record and link move under one lock and the wire is a third thing outside it, so a failed restart leaves **fail-open in the one direction that matters**: `build` → `sealed` reads `sealed` everywhere while the proxy still serves `build`. Recommended: refuse before writing either half, *and* let the module grant exactly that restart — not a path unit |
