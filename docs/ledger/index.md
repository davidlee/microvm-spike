...
| [39](./039-a-bind-is-not-a-traversal.md) | a bind is mounted as root and opened as the user | **fixed and asserted at eval, switch owed** — every `capsule-proxy-<slot>` was unable to start: `BindReadOnlyPaths` named its allowlist under `stateDir` (`0750 owner:users`) and the proxy runs as `capsule-proxy`, so the mount succeeded as root and the open failed as the unit. The link is in its own `allowlistDir` now, which is [item 36](./036-a-policy-is-selected-not-named.md)'s separation made true of the filesystem; `hostModuleUnits` **throws** when a unit binds a path under a module-declared directory its user cannot traverse, watched red on the pre-fix path. It survived a switch, 28 cases and two wire proofs because **a control can be switched and proven and still never have been started** |
